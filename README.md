...
- **Chat agent** — natural-language entry ("spent $42 at Metro on groceries"), a receipt photo/PDF, or a **CSV/XLSX/PDF statement**; streaming replies, inline generative charts/tables. Before recording, the agent confirms the target profile (when 2+ exist) and the category/sub-category. Drop a statement and it reviews the rows, proposes/creates categories, and records them — with two checkpoints (organize categories, then record) so nothing is written until you confirm.
...
- **Statement imports** — upload CSV/XLSX/PDF bank statements two ways: (1) the **review grid** — pick the target profile, edit each row inline (category/sub-category, type, total, loan, notes, receipt link), and approve; or (2) **in chat** — drop the file on the agent and it reviews, organizes categories, and records conversationally (web chat only for now). Either way, likely duplicates are flagged and pre-skipped, and recording is per-row fault-tolerant.
